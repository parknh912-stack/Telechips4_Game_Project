    ...
    for (int i = 0; i < ALIENS_N; i++)
    {
        ...
        for (int j = i + 1; j < ALIENS_N; j++)
        {
            ...
            if (collide_circle(aliens[i].cx, aliens[i].cy, r_i, aliens[j].cx, aliens[j].cy, r_j))
            {
                ...
                // 3. 겹친 정도(Overlap) 계산
                // 두 반지름의 합에서 현재 거리를 뺀 만큼이 겹친 거리입니다.
                float overlap = (r_i + r_j) - dist;

                // 4. 방향 벡터 정규화 및 밀어내기 (0.5씩 공평하게)
                float nx = dx / dist;
                float ny = dy / dist;
                ...
                // 5. 이동 후 중앙 좌표(cx, cy) 즉시 갱신 (다음 루프의 정확도를 위해)
                aliens[i].cx = aliens[i].x + (ALIEN_W[aliens[i].type] / 2.0f);
                aliens[i].cy = aliens[i].y + (ALIEN_H[aliens[i].type] / 2.0f);
...
void boss_update(void)
{
    ...
    for (int i = 0; i < BOSS_N; ++i)
    {
        ...
        {
            int new_x = -350;    
            int new_y = -800;    // 화면 y축 밖
            
            boss.x = new_x;
            boss.y = new_y;

            boss.cx = new_x + (ALIEN_W[ALIEN_TYPE_BOSS] / 2.0f);
            boss.cy = new_y + (ALIEN_H[ALIEN_TYPE_BOSS] / 2.0f);
...
void boss_draw()
{
    for (int i = 0; i < BOSS_N; ++i)
    {
        ...
        /*al_draw_scaled_rotated_bitmap(sprites.boss,
            ALIEN_W[3] / 2, ALIEN_H[3] / 2,
            boss.cx, boss.cy,
            0.5, 0.5,
            between_angle,
            0);*/
...
// 작성자: 0329 신제현
// aliens_collide() 함수와 동작 방식은 매우 비슷
// 다만 일부 속성값의 변경이 존재
// 혹시 보스가 2개 이상 나올 경우를 대비한 함수
void boss_collide(void)
{
    // (작성 보류)
}